...
class Pi0DiscreteFlowARConfig(_model.BaseModelConfig):
    dtype: str = "bfloat16"
    ...
    @override
    def create(self, rng: at.KeyArrayLike) -> "Pi0DiscreteFlowAR":
        return Pi0DiscreteFlowAR(self, rngs=nnx.Rngs(rng))
...
class Pi0DiscreteFlowAR(_model.BaseModel):
    def __init__(self, config: Pi0DiscreteFlowARConfig, rngs: nnx.Rngs):
    ...
    def compute_loss(
        ...
        *,
        ...
        # we define the mask style
        num_to_keep = jnp.floor(time * seq_len).astype(jnp.int32)
        indices = jnp.arange(seq_len)
        tokens_to_keep = indices < num_to_keep[:, None]
    ...
    def sample_actions(
        ...
        *,
        ...
        """
        Samples actions using iterative decoding, constrained to a left-to-right schedule.
        """
        decode_rng = rng
        ...
        def loop_body(i, carry):
            ...
            # a. Calculate masking schedule (unchanged).
            ...
            # b. Embed current (partially masked) tokens and time (unchanged).
            ...
            # c. Create attention mask and run the model forward pass (unchanged).
            suffix_ar_mask = jnp.zeros(action_seq_len, dtype=jnp.bool_)
            suffix_ar_mask = suffix_ar_mask.at[0].set(True)
            ...
            logits = self.head_out_proj(suffix_out)

            # d. Get predicted token IDs from logits (unchanged).
            ...
            # --------------------------------------------------------------------------
            # This block replaces confidence-based token selection with deterministic,
            # left-to-right unmasking to align with the new training objective.

            # e. Determine how many tokens to unmask based on the cosine schedule.
            ...
            num_to_unmask = jnp.maximum(1, num_to_unmask) # Ensure progress
            # In the final step, unmask all remaining tokens.
            num_to_unmask = jnp.where(i == num_steps - 1, num_masked_currently, num_to_unmask)

            # f. Find the *first k* masked indices to unmask, not the most confident.
            # We achieve this by creating a cumulative sum over the mask of tokens to be predicted.
            # The first k `True` values in `mask_to_be_predicted` will have a cumsum value from 1 to k.
            masked_indices_cumsum = jnp.cumsum(mask_to_be_predicted, axis=-1)
            
            # The update mask selects the tokens that are currently masked AND fall within the
            # first `num_to_unmask` positions among the masked tokens.
            unmask_update_mask = jnp.logical_and(
                masked_indices_cumsum <= num_to_unmask[:, None],
                mask_to_be_predicted
            )
            # --------------------------------------------------------------------------
            # [MODIFIED BLOCK END]

            # g. Update the action tokens and the mask of tokens to be predicted.
            # This logic remains the same, but operates on the new `unmask_update_mask`.
