...
def create_animation(records: List[dict], output_path: str, fps: int = 5) -> None:
    ...
    def animate(frame):
        ...
        ax1.plot(current_positions[:, 0], current_positions[:, 1], current_positions[:, 2], 
                'b-', linewidth=2, alpha=0.7)
        ax1.scatter(current_positions[-1, 0], current_positions[-1, 1], current_positions[-1, 2], 
                   color='red', s=100)
        
        ax1.set_xlabel('X')
        ax1.set_ylabel('Y')
        ax1.set_zlabel('Z')
        ax1.set_title(f'3D Trajectory (Step {frame})')
    ...
    # 创建动画
    anim = FuncAnimation(fig, animate, frames=len(records), interval=1000//fps, repeat=True)
    
    # 保存动画
    pathlib.Path(output_path).mkdir(parents=True, exist_ok=True)
    anim.save(f"{output_path}/policy_animation.gif", writer='pillow', fps=fps)
    print(f"Saved animation to {output_path}/policy_animation.gif")
